...
function processRadioButtons(event) {
  if ((debuglevel & 1+32) == 1+32) { console.log("FUNCTION processRadioButtons() START"); }
...
async function loadCachedData() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION loadCachedData() START"); }
  ...
  if ((debuglevel & 1) == 1) { console.log("FUNCTION loadCachedData() EXIT"); }
}

async function reloadDatafromStorage() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION reloadDatafromStorage() START"); }
  cachedData = [];
  let tmp = await loadDatafromStorage();
  cachedData = JSON.parse(tmp["AnticontainerData"]);
  clearHTMLOptionsList();
  buildHTMLOptionsList(cachedData);
  if ((debuglevel & 1) == 1) { console.log("FUNCTION reloadDatafromStorage() EXIT"); }
}
...
function clearAllStorage() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION clearAllStorage() START"); }
  clearStorage("CachedData");
  clearStorage("AnticontainerData");
  clearHTMLOptionsList();
  if ((debuglevel & 1) == 1) { console.log("FUNCTION clearAllStorage() EXIT"); }

  function clearStorage(storageName) {
    if ((debuglevel & 1) == 1) { console.log("FUNCTION clearStorage() START"); }
    ...
    if ((debuglevel & 1) == 1) { console.log("FUNCTION clearStorage() EXIT"); }
  }
}
...
function clearHTMLOptionsList() {
    ...
    (elem  => {
      if ((debuglevel & 32) == 32) { console.log("removing "+elem.id); }
      elem.remove();
    }));
}
...
function emptyRule() {
  if ((debuglevel & 1) == 1)  { console.log("FUNCTION emptyRule() START"); }
  ...
  if ((debuglevel & 1) == 1)  { console.log("FUNCTION emptyRule() EXIT"); }
}
...
function deleteRule() {
  if ((debuglevel & 1) == 1)  { console.log("FUNCTION deleteRule() START"); }
...
// remove the object from the array
  ...
  emptyRule();
  ruleNameChanged();
  if ((debuglevel & 1) == 1)  { console.log("FUNCTION deleteRule() EXIT"); }
}
...
function buildRule(ruleID) {
  if ((debuglevel & 1) == 1)  { console.log("FUNCTION updateRule() START"); }
  var newrule = {};
  var errormessage = "";
...
// If not, insert or update a rule <object> as appropriate.
    ...
    if ((debuglevel & 32) == 32) { console.log("Updating Rule ID:  = "+ruleID); }
...
//     document.getElementById("ruleslist").dispatchEvent(event);
    if ((debuglevel & 1) == 1) { console.log("FUNCTION performUpdateRule() EXIT"); }
...
function listItemChanged() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION listItemChanged() START"); }
...
// index is the position of that item in the cachedData array
  if ((debuglevel & 32) == 32) { console.log("User has clicked Item:"+this.value); }
...
// Update the form buttons
  ...
  if ((debuglevel & 1) == 1) { console.log("FUNCTION listItemChanged() EXIT"); }
}
...
async function showHistory() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION showHistory() ENTER"); }
  await writedays();
  let open=await getDownloadWindowId(32);
  if ((debuglevel & 32) == 32) { console.log(open);}
/*
...
//}
  if ((debuglevel & 1) == 1) { console.log("FUNCTION showHistory() EXIT"); }
}

function addNewCleaner() {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION addNewCleaner() ENTER"); }
  let count=document.getElementById("cleanersdiv").childElementCount / 4;
  createCleanerItem("","",count + 1);
  if ((debuglevel & 1) == 1) { console.log("FUNCTION addNewCleaner() EXIT"); }
}

function updateCleaners(cleanerArray) {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION updateCleaners() ENTER"); }
  removeHTMLCleaners();
  let ptr = 1;
  cleanerArray.forEach(elem  => {
    createCleanerItem(elem.pattern,elem.replacement,ptr++);
  });
  if ((debuglevel & 1) == 1) { console.log("FUNCTION updateCleaners() EXIT"); }
}

function createCleanerItem(pattern, replacement, id) {
  if ((debuglevel & 1) == 1) { console.log("FUNCTION createCleanerItem() ENTER"); }
  let box = document.getElementById("cleanersdiv");
  ...
  if ((debuglevel & 1) == 1) { console.log("FUNCTION createCleanerItem() EXIT"); }
}
